...
local Project = setmetatable(M, { ---@type Project
  __index = function(self, k)
    ...
    if Util.mod_exists('project.' .. k) then
      return Util.rawset(self, k, require('project.' .. k))
    end
    if k == 'delete_project' then
      return Util.rawset(self, k, require('project.util.history').delete_project)
    end
    if k == 'get_config' then
      return Util.rawset(self, k, require('project.config').get_config)
    end
    if k == 'get_history_paths' then
      return Util.rawset(self, k, require('project.core').get_history_paths)
    end
    if k == 'get_last_project' then
      return Util.rawset(self, k, require('project.core').get_last)
    end
    if k == 'get_project_root' then
      return Util.rawset(self, k, require('project.core').get_project_root)
    end
    if k == 'get_recent_projects' then
      return Util.rawset(self, k, require('project.util.history').get_recent_projects)
    end
    if k == 'rename_project' then
      return Util.rawset(self, k, require('project.util.history').rename_project)
    end
    if k == 'root_files' then
      return Util.rawset(self, k, require('project.core').root_files)
    end
    if k == 'run_fzf_lua' then
      return Util.rawset(self, k, require('project.extensions.fzf-lua').run)
    end
    if require('project.popup')[k] then
      return Util.rawset(self, k, require('project.popup')[k])
    end
  end,
})
